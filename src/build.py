...
def build_clips():
  ...
  function renderHero(){
    ...
    var heroMedia=hemb
      ?'<div class="orig-feat"><iframe src="'+esc(hemb)+'" title="'+esc(feat.title||'클립')+'" allow="autoplay; fullscreen; encrypted-media; picture-in-picture" allowfullscreen loading="lazy" style="position:absolute;inset:0;width:100%;height:100%;border:0"></iframe></div>'
      :'<a class="orig-feat" href="clip.html?id='+encodeURIComponent(feat.id)+'" aria-label="'+esc(feat.title)+' 시청">'+bgOf(feat)+'<div class="orig-play" aria-hidden="true"><svg width="1em" height="1em" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M8 5v14l11-7z"/></svg></div></a>';
  ...
  function card(c){
    var emb=window.WhaleEmbed?window.WhaleEmbed(c.url):null;  // 임베드 가능하면 SOOP/YouTube 포스터+재생 자동(썸네일 저장 불필요)
    var media=emb
      ?'<iframe src="'+esc(emb)+'" title="'+esc(c.title||'클립')+'" allow="autoplay; fullscreen; encrypted-media; picture-in-picture" allowfullscreen loading="lazy" style="position:absolute;inset:0;width:100%;height:100%;border:0"></iframe>'
