...
> **v4.2(2026-07-27 기능추가·버그헌트판)**: 홈 멤버 단일 소스화 · 클립/아카이브 검색·필터 · 아카이브→클립 연결
> 실동작화(기록에 tags 전무 → 제목 매칭) · 홈 베스트 클립 실데이터 · `WhaleEmbed`를 `assets/embed.js`로 분리 ·
> **사이트 자체 조회수(clipviews 규칙 + A안)** · "역할극/공식" 제거. Chromium 헤드리스 전수 검사로
> **없는 클립 id 폴백 오집계**·**og 삭제 재발** 등 4건 수정. 상세 §0-8, 검증 부록 C-4·C-5, 리스크 3종(§7), 백로그 E-10~12.
> **v4.3(2026-07-28 필터 UX·연결·인사기록카드판)**: 필터 select 다크화(`color-scheme`) · 선택지 축약(멤버=현재 16인,
> 게임=3회 이상, 나머지 '기타') · **클립↔아카이브 양방향 이동**(판정을 `linksTo` 한 벌로 통합) ·
> 클립 태그를 **검색해서 고르는 콤보박스**(후보=기록 제목) · **인사기록카드 더미 스탯 → 생일·데뷔일·입사일 + D-day**.
> 검수에서 **사본 불일치 버그 2건**(홈 인라인 모달 JS·멤버 상세 스탯) 발견·수정. 상세 §0-9.
...
### 0-8. 2026-07-27 기능 추가 + 전수 버그헌트 (v4.2)
**추가/변경 (전부 커밋 1건)**
- **홈 멤버 단일 소스화**: `index.html`의 손복사 MEMBERS 배열(옛 직급·8개 가짜 부서)을 `members.json`에서
  자동 생성(`build.py sync_home_members`, `@members:start/end` 마커). 홈=크루=멤버 페이지 16인·순서·8필드 일치.
- **클립 검색(`?q=`) + 태그 필터(`?t=`)**: 제목·제작자·카테고리·설명·태그 훑기, 공백=AND. 검색창은 `#cChips`
  **바깥**(칩 재렌더가 입력값·포커스를 날리므로) + `display:contents`로 한 줄 유지.
- **아카이브 검색·필터**: 직접입력(`?q=`, 제목·날짜·멤버·게임·상대크루) + 참여 멤버(`?m=`)·게임(`?g=`)·
  결과(`?w=win|other`) + 초기화. 선택지는 **기록에 실제 등장한 값**에서 생성(졸업 온자두·게스트 빵땅콩/마늘빵/미현영 포함 18명·21종).
- **아카이브→클립 연결 실동작화**: 운영 기록 15건에 `tags`가 **전무**해 '관련 클립'이 여태 0건이었음(코드는 있었으나 데이터가 없어 사문).
  `keywordsOf()`=태그+**제목**으로 매칭 → 15건 중 10건 연결. '관련 클립 보러가기'는 `clips.html?t=`(정확일치)로 이동해 **같은 집합** 보장.
- **홈 베스트 클립 실데이터화**: 정적 4건 → 등록 클립(대표→조회수→최신 등록순). 실패 시 정적 폴백 유지.
- **`WhaleEmbed` 분리**: `site.js` → **`assets/embed.js`**(홈은 site.js 미로드라 공유 불가였음). 소비자=홈+하위 페이지.
- **사이트 자체 조회수 도입**: `/rework/clipviews/{clipId}` 신규 노드 + 규칙 "1씩 증가만" 서버 강제.
  `WhaleViews.load/bump`(site.js), 홈은 인라인 합류. 표기 = **"사이트 조회 N"**(SOOP 조회수와 구분).
- **"역할극"·"공식" 전면 제거**: 사이트 파일 0건(문구 단위 치환). ROADMAP의 "공식"은 *Firebase 공식 문서/한계선 공식(수식)* 이라 **의도적 제외**.

**버그헌트로 잡은 것 (Chromium 149 헤드리스 실측)**
- 🔴 **없는 클립 id → 1번 클립 폴백 + 그 클립 조회수 증가**(실측 3→4). 폴백 제거 → "클립을 찾을 수 없습니다"(아카이브 상세와 동일 처리). 재검증 4→4, 정상 id·legacy `?i=` 회귀 없음.
- 🔴 **og 7종 삭제 재발**(v4.0 §0-6에 이어 2회차). 전 페이지 `og:image`가 죽은 채 커밋 대기였음 → `git restore`로 복원. **커밋 전 `git status`에서 `D og*.png` 확인이 상시 절차가 되어야 함**.
- 🟡 인라인 `<script>` JSON에 `</` 이스케이프 누락(현재 데이터엔 무해) → `<\/` 처리.
- 🟡 임베드 iframe 로딩 중 흰 배경 → 다크 배경 지정. **헤드리스에선 여전히 흼**(SOOP 문서 자체 배경) — 실브라우저 확인 필요.

**검증 (전부 PASS)**: 규칙 실서버 10/10(정상 +1만 통과, 건너뛰기·감소·문자열·소수점·삭제·잘못된키·**상위경로 PUT/PATCH 우회** 전부 denied) ·
조회수 e2e(null→1→2) · 자체검사 35건 · 빌드 멱등 · 20파일 구문 · **링크 감사 20페이지 깨진링크 0/죽은 런타임링크 0/noopener 누락 0** ·
멤버카드 16/16 상세 인물 일치 · 악성 URL 파라미터 8종 주입 0 · Firebase 전면불통/조회수노드만 실패 폴백 정상.

---

### 0-9. 2026-07-28 필터 UX · 양방향 연결 · 인사기록카드 (v4.3)

**추가/변경**
- **필터 select 디자인 시스템 정합**: 펼친 목록이 하얀 윈도우 기본 UI로 나오던 문제 —
  `<select>`는 닫힌 상태만 CSS로 칠할 수 있고 목록은 OS/브라우저가 그리므로 **`color-scheme:dark`가 유일한 표준 스위치**.
  + `appearance:none` + 인라인 SVG 화살표(chip과 동일 톤), `option{background}`는 리눅스·구형 엣지 보강,
  선택 중이면 `--brand-soft` 배경. `.pg-select` 공용이라 클립 태그 필터도 같이 적용됨.
- **아카이브 선택지 축약** (`build.py` `fillOptions`): 멤버 = **members.json 16인 중 기록에 등장한 사람만**(정렬은 rank 순,
  졸업·게스트는 `기타`), 게임 = **`GAME_MIN=3`회 이상**만 많이 한 순(나머지 `기타`). `matches()`의 `기타`는
  "명단/상위목록 **밖** 값이 낀 기록"으로 판정. v4.2의 18명·21종 나열이 이걸로 대체됨.
- **클립 ↔ 아카이브 양방향**: `tagsOf/keywordsOf/linksTo`를 `_LINK_JS_SHARED`로 묶어 기록 상세('관련 클립')와
  클립 상세('관련 기록')가 **같은 판정 한 벌**을 공유. 한쪽만 고쳐 A→B는 보이는데 B→A는 안 보이는 비대칭을 구조적으로 차단.
  클립 상세는 `D.list('contests')`를 함께 받아(실패해도 재생은 유지) `archive-detail.html?id=`로 링크.
- **클립 태그 = 검색 콤보박스**: 후보 = **아카이브 기록 제목·태그 + 이미 쓰인 클립 태그**(기록 제목이 곧 연결 키라
  고르기만 하면 연결 성립 — 오타로 끊기던 경로 제거). 칩 표시·×제거·Backspace·Enter로 새 태그,
  대소문자/공백 중복 차단. 저장 형식은 종전 쉼표 문자열(hidden input) 유지 → **제출·RTDB 규칙 무변경**.
- **인사기록카드 실데이터화**: `members.json`의 더미 `stats`(누적방송시간·클립수·누적시청자) 삭제 →
  `birth`(MM-DD)·`debut`·`joined`(YYYY-MM-DD) 16인분. 출처 = 운영자 '카운트다운 시트지'
  (`docs.google.com/spreadsheets/d/1O10gZ…`), CSV로 받아 **48개 값 전수 대조 일치**.
  표시 = 날짜 + 아래 작게 `D-279`/`D+6072` + 경과 `16년 7개월 16일`. **브라우저 오늘 기준 자동 재계산**(시트 갱신 불필요).
  날짜만 다루므로 UTC 자정 기준 감산(시간대·DST로 하루 밀림 방지), 연·월·일 경과는 달력 기준(달 길이가 달라 나눗셈 불가).

**검수로 잡은 것 — 전부 "같은 화면이 코드로는 여러 벌"에서 나옴**
- 🔴 **홈 인사기록카드 전체 미표시**: 홈은 `site.js`를 의도적으로 로드하지 않아 **모달 JS 인라인 사본**을 따로 보유 →
  `site.js`만 고쳤으면 `m.stats.forEach`가 TypeError. 인라인 사본도 동일 교체 +
  **두 사본 문자열 동일성을 `test_pages.js`가 대조**(사람 기억 대신 테스트로 고정).
- 🔴 **멤버 상세(`member.html`) 스탯 3칸이 빈 상자로 남음**: 이 페이지도 `m.stats`를 자체 렌더 중이었음 →
  같은 3칸으로 통일하고, 계산은 `window.WhaleDate`로 내보내 **사본을 늘리지 않고** 공유.
- 🟡 `WHALE_MEMBERS` 주입에 `</` 이스케이프 누락(홈 동기화에만 있던 처리) → 하위 페이지·ROSTER 주입에도 적용.
- 🟡 `member.html`의 죽은 `countUp`/`PRM` 삭제(스탯 카운트업 전용이었음), 태그 후보 버튼 `role="option"`.
- ⚪ **미조치(기존 백로그)**: `build.py` `member_modal()`은 `need_member_modal=True`가 어디서도 안 쓰여
  **어느 페이지에도 렌더되지 않는 죽은 코드**(인사기록카드 모달은 홈 전용). E-6b/F-5 이월 항목이라 일관성만 맞춰둠.

**검증 (전부 PASS)**: 자체검사 **65건**(클립검색 8·아카이브필터 17·아카이브↔클립 11·태그 콤보박스 5·홈정렬 5·
...
- [x] **2.6-10. Q6 키교체 재상신** (W-1) — ✅**2026-07-28(D-6) 상신 완료 → 사용자 "추후처리" = 재보류 확정.**
  **→ §5-5 F-2로 최종 이관**(D+2주 내 1회 권장). 이 항목은 이로써 종료. 상신 때 정리한 위험 범위는 아래 보존:
  ⚠️07-08의 "격리된 `/rework`만 영향" 판단은 실제보다 위험을 작게 본 것 — **SA 키는 RTDB 규칙을 통째로 우회**하므로
  `.write:false`인 **`/permissions`에 자기 닉을 admin으로 등록**할 수 있다(= 사이트 관리자 권한 획득).
  cutover로 `/rework`가 실서비스가 되면 "격리 경로 문제"에서 "사이트 전권 문제"로 승격된다. §7 리스크 최고 잠재영향 유지.
  (원 상신 내용) 근거: `/rework`가 실서비스로
...
- [ ] **4-5. 실로그인 검증 (사용자 + Claude 안내)** — **Phase B의 첫 실전 검증 지점**
  0. **§1-4 처리 — 시드/테스트 데이터 청소 3건**(admin 로그인 상태, 개시 공지 게시 전에 먼저).
     ...
     - ~~더 빠른 대안: D-Day 전 콘솔 삭제~~ → **2026-07-28 사용자 "추후처리" = D-Day 당일 이 단계에서 처리 확정.**
...
- ~~**E-1. 클립 임베드 플레이어 확대**~~ ✅**완료(§2.6-2 + v4.2)**. ⚠️**코드 앵커 변경**: `embedUrlOf`는 이제
  `site.js`가 아니라 **`assets/embed.js`**(홈은 site.js를 로드하지 않아 공유가 불가능했음). 소비자 = 홈 + 하위 페이지.
  부록 C-2 재검증은 `assets/embed.js`를 대상으로 한다.
- **E-10. 임베드 façade 전환(제3자 추적 차단)** ⭐v4.2 신설, §7 리스크 대응 — 목록/히어로에서 iframe을 걷어내고
  **클릭 시에만** 임베드 삽입(포스터는 그라데이션+제목). 노출면 14개→0개(첫 화면). 대가 = 카드에 영상 썸네일이 사라짐.
  - 착수 트리거: 개인정보처리방침에 제3자 전송을 명시하지 않기로 하면 즉시. 합격 = 클립 목록 로드 시 `connect.facebook.net` 요청 0.
- **E-11. 조회수 운영** ⭐v4.2 신설 — 현재 A안(규칙 강제 카운터, 조작 가능). ①위조 관측 시 워커+KV(B안) 승격
...
| **조회수 위조(미인증 +1 반복)** | 중 | 홈 히어로 노출 순위 왜곡 | v4.2 A안의 알려진 천장. 규칙이 임의값·감소·삭제·상위경로는 차단. 관측 시 워커 경유(B안)로 승격 |
| **SOOP 임베드가 Meta 픽셀 로드** | 고(상시) | 방문자 IP·UA·Referer가 제3자로 전송 | 목록 1회에 iframe 14개. 클릭 시에만 임베드하는 진짜 façade로 전환하거나 처리방침에 제3자 전송 명시 |
| **og 이미지 삭제 재발** | 중 | 전 페이지 링크 미리보기 404 | 2회 발생(v4.0, v4.2). 커밋 전 `git status`에 `D og*.png` 확인(부록 D) |
...
```bash
B=https://whaie-corp-default-rtdb.asia-southeast1.firebasedatabase.app/rework/clipviews
ID=<실제 클립 push key>; N=$(curl -s $B/$ID.json); N=${N/null/0}
curl -s -X PUT -d "$((N+1))"  $B/$ID.json   # 유일한 성공 케이스
curl -s -X PUT -d "$((N+50))" $B/$ID.json   # 이하 전부 Permission denied 여야 함
curl -s -X PUT -d "$((N-1))"  $B/$ID.json
curl -s -X PUT --data-binary '"2"' $B/$ID.json
curl -s -X PUT --data-binary '1.5' $B/$ID.json
curl -s -X DELETE $B/$ID.json
curl -s -X PUT -d 1 $B/hack.json                          # 키 형식 위반
curl -s -X PATCH --data-binary "{\"$ID\":99}" $B.json     # 상위 경로 우회
curl -s -X PUT   --data-binary "{\"$ID\":99}" $B.json
```
...
# 페이지 로직 자체검사 35건 (부록 C-4 — 빌드 후 항상)
node src/test_pages.js
# ⚠️커밋 전 필수: og 이미지 삭제 재발 확인(2회 발생) — 나오면 git restore
git status --short | grep "^ D .*og.*png"
