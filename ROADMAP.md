...
- [ ] **2.5-3. news 피드 런타임 전환**(§2-A-5/§5-4) — 정적 시드 → `WhaleData.list` 로드로 전환해 cutover 직후부터 실데이터 반영.
- [ ] **2.5-4. 콘텐츠 사전 준비** — 개시 공지 문구 초안, 클립 후보 목록(URL+태그) 선정. 입력 자체는 cutover 후 admin UI지만 준비는 미리(사용자 주도, Claude 보조).
- [ ] **2.5-5. 미세정리 일괄** — crew.html "대표이사/신입사원" 옛 표현(2-A-1 잔여), admin-* 데모 목업 옛 부서명, index 모바일 레이아웃뷰포트 466px(Swiper 초기화 전, 경미).
