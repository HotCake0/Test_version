# 고래상사 리워크 — 로드맵 v3.2 (2026-07-12 담금질 실행계획)
...
| **07-12 디자인 4R (체감 품질)** | 7항목: 런타임 6페이지+홈 **스켈레톤 로딩**(.skel 셔머, CLS 자리 예약) · 목록 이미지 lazy/decoding · **tabular-nums 전역**(body) · 멤버카드 **3D 틸트+글레어**(hover+PRM 가드, is-tilt) · **press scale 피드백**(:active .97) · **공유 요소 전환**(멤버카드→상세 초상 member-hero, 클릭 시 JS 부여+상세는 CSS 고정) · **빈 상태 고도화**(클립/일정/공지/아카이브 아이콘+제목+안내 .n-empty). 검증: ast+재빌드 19p·인라인 45스크립트 node --check 0실패·HTTP 20/20 | `52f3e08` |
...
- [ ] **2.6-1. 히어로 일정 스트립 LIVE 실판별** (P1, 실행자 Claude, ~1h) — 07-12 미결②의 해소.
...
- [ ] **2.6-2. 클립 인라인 재생** (P2, Claude, ~2h) — §5-4 "임베드 확대" 앞당김.
...
- [ ] **2.6-3. 필터 URL 동기화** (P3, Claude, ~1h) — critique 잔여.
...
- [ ] **2.6-4. dead CSS 정리 + stats 오류문 순화** (P3, Claude, ~1h) — critique 잔여.
...
- [ ] **2.6-7. Lighthouse 기준선 측정** (P2, Claude, ~1h) — §5-5 분기 점검의 비교 기준 확보.
...
- [ ] **2.6-8. 디자인 4R 최종 critique** (P2, Claude, W-1 권장) — D-Day 직전 나침반.
  - 절차: impeccable 듀얼 에이전트 1회(기준선 27/40). **P1만 즉시 수정, P2 이하는 §5 백로그 기록만**(D-7 이후 큰 변경 금지).
  - 합격: P1 0건 상태로 D-Day 진입.
...
| **E** | 수시(수요 기반) | 기능/콘텐츠 확장 | §5-4 | push만 |
| **F** | D+1주부터 상시 | 운영 위생(백업 자동화·키 로테이션·Lighthouse) | §5-5 | 워커 cron 1회 |
...
- **E-4. 태그 자동완성** (§7 리스크 "태그 오타" 완화): 클립/아카이브 폼 태그 입력에 기존 태그 datalist 제안
  (`D.list` 결과에서 태그 수집). 소품 — E-1/2 하는 김에.
